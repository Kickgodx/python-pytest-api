...
class UserAPI(CustomRequester):
    ...
    def create_user_with_array(self, client: Client, data: list):
        ...
        return self.post(Router.POST_CREATE_USER_WITH_ARRAY, data=data, headers=client.get_base_headers())

    def get_user_by_username(self, client: Client, username: str):
        """Get user by username"""
        return self.get(Router.GET_USER_BY_USERNAME.format(username=username), headers=client.get_base_headers())
    ...
    def update_user(self, client: Client, username: str, data):
        """Update user"""
        return self.put(Router.PUT_UPDATE_USER.format(username=username), data=data, headers=client.get_base_headers())

    def delete_user(self, client: Client, username: str):
        """Delete user"""
        return self.delete(Router.DELETE_USER.format(username=username), headers=client.get_base_headers())
